...
def generate_pdf_report(
    ...
    story.append(Paragraph("04  ·  FEATURE ANALYSIS", s["section_label"]))
    story.append(Paragraph("SHAP Feature Importance", s["section_title"]))
    story.append(Paragraph(
        "SHAP (SHapley Additive exPlanations) values show which features "
        "most influence the model predictions. Features with high SHAP values "
        "are the primary drivers of potential bias.",
        s["body"]
    ))
    story.append(Spacer(1, 3 * mm))

    shap_path = "assets/shap_global.png"
    if os.path.exists(shap_path):
        from reportlab.platypus import Image as RLImage
        img = RLImage(shap_path, width=PAGE_W - 2 * MARGIN, height=80 * mm)
        story.append(img)
    else:
        story.append(Paragraph(
            "SHAP chart not available — run explain.py to generate.",
            s["body"]
        ))
    story.append(Spacer(1, 6 * mm))
    story.append(_divider())
    ...
    # ── SECTION 6: AI ETHICS REPORT ──────────────────────────────────────────
    story.append(PageBreak())
    story.append(Paragraph("05  ·  AI ETHICS REPORT", s["section_label"]))
    story.append(Paragraph("Generated by Gemini AI", s["section_title"]))
    story.append(Paragraph(
        "The following analysis was automatically generated by Google Gemini AI "
        "based on the bias metrics computed above.",
        s["body"]
    ))
    story.append(Spacer(1, 3 * mm))
    ...
    if ai_report:
        ...
        story.append(Paragraph(
            "AI report not available. Run the audit from the dashboard to generate.",
            s["body"]
        ))
    story.append(Spacer(1, 6 * mm))
    story.append(_divider())
